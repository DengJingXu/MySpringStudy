...
public class AccountDaoImpl2 extends JdbcDaoSupport implements IAccountDao {
    ...
    public Account getAccountById(int accountId) {
        List<Account> accounts = super.getJdbcTemplate().query("select * from account where account_id=?", new BeanPropertyRowMapper<>(Account.class), 7);
        return accounts.isEmpty()?null:accounts.get(0);
    }

    @Override
    public Account getAccountByName(String accountName) {
        List<Account> accounts =  super.getJdbcTemplate().query("select * from account where account_name=?", new BeanPropertyRowMapper<>(Account.class), "lisi");
        if(accounts.isEmpty()){
            return null;
        }
        if(accounts.size()>1){
